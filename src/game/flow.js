...
import { startBidding, submitBid, biddingResolution, legalBidOptions, currentBidder, } from './bidding';
import { applyEndgameRule, isGameOver, scoreHand, } from './scoring';
import { HUMAN_SEAT, nextSeat, teamOf, } from './state';
const HAND_SIZE = 5;
const KITTY_SIZE = 3;
...
export function makeInitialState(seed, settings) {
    const dealer = 3; // first deal: dealer to right of you (seat 3); you (0) bid first
    const state = {
        rngSeed: seed,
        phase: 'intro',
        dealer,
...
export function dealHand(state) {
    ...
    // 3 to kitty.
    for (let i = 0; i < KITTY_SIZE; i++)
        state.kitty.push(deck[idx++]);
...
export function getCurrentBidderSeat(state) {
    if (!state.bidding)
        return null;
    return currentBidder(state.bidding);
}
export function getLegalBidOptions(state) {
    if (!state.bidding)
        return [];
    return legalBidOptions(state.bidding, state.dealer);
}
/** Submit a bid for the current bidder. Advances phase if bidding resolves. */
export function submitBidAction(state, seat, option) {
    if (!state.bidding)
        throw new Error('No bidding state');
 ...
 */
export function setTrumpAndTakeKitty(state, trump, bidderDiscards) {
...
export function discardAndDraw(state, seat, discards) {
    if (state.phase !== 'discard')
        throw new Error('Not in discard phase');
    if (state.discardQueue[0] !== seat)
        throw new Error(`Not seat ${seat}'s turn to discard`);
...
export function playCard(state, seat, card) {
    ...
    if (state.contract.amount === 30 &&
        ...
        if (hasTrump && !playedIsTrump)
            throw new Error('Bid-30: opponent must lead a trump');
    }
    ...
    if (state.currentTrick.plays.length === 4) {
        // Trick complete.
        const winnerLocalIdx = trickWinnerIndex(state.currentTrick.plays.map((p) => p.card), state.trump);
...
export function resolveScorePhase(state) {
    if (state.phase !== 'score')
        throw new Error('Not in score phase');
    if (!state.contract || !state.trump)
        throw new Error('Bad state');
    const result = scoreHand(state.completedTricks, state.trump, state.contract);
    state.totalScore = applyEndgameRule(state.totalScore, result.pointsByTeam, state.contract, state.settings.inHoleVariant);
    state.handsPlayed++;
    state.log.push(`Hand result: bidder ${state.contract.bidder} (team ${teamOf(state.contract.bidder)}) ` +
