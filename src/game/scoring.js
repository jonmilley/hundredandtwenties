 ...
 */
export function scoreHand(tricks, trump, contract) {
    ...
    return {
        pointsByTeam: final,
        bidMade,
        bestTrumpSeat: best ? best.seat : null,
 ...
 * is waived.
 */
export function applyEndgameRule(current, delta, contract, inHoleVariant) {
    const next = [current[0] + delta[0], current[1] + delta[1]];
    if (!contract)
        return next;
    for (const team of [0, 1]) {
        if (current[team] < 100)
            continue; // rule only applies at 100+
        if (next[team] < 120)
            continue; // not crossing 120 anyway
        const isBidderTeam = teamOf(contract.bidder) === team;
        if (isBidderTeam)
            continue; // crossed via successful bid -> ok
        // Defending team would cross 120 via tricks alone. Apply cap unless waived.
        const otherTeam = team === 0 ? 1 : 0;
        if (inHoleVariant && next[otherTeam] < 0)
            continue; // variant waives
        next[team] = 119;
    }
    return next;
}
...
export function gameWinner(score) {
    if (score[0] >= 120 && score[0] > score[1])
        return 0;
    if (score[1] >= 120 && score[1] > score[0])
        return 1;
    return null;
}
/** Convenience: also handles in-hole rule for an existing GameState. */
export function applyHandResultToState(state, result) {
    return applyEndgameRule(state.totalScore, result.pointsByTeam, state.contract, state.settings.inHoleVariant);
}
