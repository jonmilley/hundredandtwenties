import { isAceOfHearts, isRedSuit } from './cards';
const PIP = {
 ...
 */
export function trumpPower(card, trump) {
    const isTrumpSuit = card.suit === trump;
    const isAH = isAceOfHearts(card);
    if (!isTrumpSuit && !isAH)
        return null;
    if (isTrumpSuit && card.rank === '5')
        return 100;
    if (isTrumpSuit && card.rank === 'J')
        return 99;
    if (isAH)
        return 98;
    if (isTrumpSuit && card.rank === 'A')
        return 97; // unreachable when hearts is trump (handled above)
    if (isTrumpSuit && card.rank === 'K')
        return 96;
    if (isTrumpSuit && card.rank === 'Q')
        return 95;
    ...
    if (isRedSuit(trump))
        return pip; // 10..2 → 10..2, well under 95
 ...
 */
export function nonTrumpPower(card, ledSuit) {
 ...
 */
export function trickWinnerIndex(plays, trump) {
    if (plays.length === 0)
        throw new Error('trickWinnerIndex: empty trick');
...
export function isRenegeEligible(card, trump) {
    if (card.suit === trump && card.rank === '5')
        return true;
 ...
 */
export function bestTrumpPlayer(trickPlays, trump) {
    let best = null;
    let bestScore = -Infinity;
