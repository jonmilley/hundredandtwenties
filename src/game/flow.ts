...
export function chooseNormalKitty(state: GameState): void {
  if (state.phase !== 'bid_on_kitty') throw new Error('Not in kitty choice phase');
  state.bidOnKitty = false;
  state.phase = 'kitty';
  state.version++;
}
 ...
 */
export function chooseOneCardKitty(state: GameState): void {
  if (state.phase !== 'bid_on_kitty') throw new Error('Not in kitty choice phase');
...
export function finalizeOneCardKittyKeep(state: GameState, keepIndex: number): void {
  ...
  if (!keptCard) throw new Error('Invalid card index');
